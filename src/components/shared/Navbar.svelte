<script lang="ts">
  import { translate, availableLocales, setLocale, locale } from '../../i18n/index.js';
...
<nav class="navbar">
  <div class="navbar-inner">
    ...
    <div class="nav-links" class:nav-open={menuOpen}>
      <a href="/" class="nav-link" class:active={currentPage === 'home' || currentPage === 'game'} onclick={handlePlayClick}>{$translate('ui.nav_play')}</a>
      <a href="#/docs" class="nav-link" class:active={currentPage === 'docs'} onclick={() => menuOpen = false}>{$translate('ui.nav_docs')}</a>
      <a href="#/changelog" class="nav-link" class:active={currentPage === 'changelog'} onclick={() => menuOpen = false}>{$translate('ui.nav_changelog')}</a>
      <a href="#/devblog" class="nav-link" class:active={currentPage === 'devblog'} onclick={() => menuOpen = false}>{$translate('ui.nav_blog')}</a>
      <a href="#/stats" class="nav-link" class:active={currentPage === 'stats'} onclick={() => menuOpen = false}>{$translate('ui.nav_stats')}</a>
      ...
      {#if availableLocales.length > 1}
        <select class="locale-picker" value={$locale} onchange={(e) => setLocale((e.target as HTMLSelectElement).value)}>
