<script lang="ts">
  import { t, availableLocales, setLocale, locale } from '../../i18n/index.js';
...
<nav class="navbar">
  <div class="navbar-inner">
    ...
    <div class="nav-links" class:nav-open={menuOpen}>
      <a href="/" class="nav-link" class:active={currentPage === 'home' || currentPage === 'game'} onclick={handlePlayClick}>{t('ui.nav_play')}</a>
      <a href="#/docs" class="nav-link" class:active={currentPage === 'docs'} onclick={() => menuOpen = false}>{t('ui.nav_docs')}</a>
      <a href="#/changelog" class="nav-link" class:active={currentPage === 'changelog'} onclick={() => menuOpen = false}>{t('ui.nav_changelog')}</a>
      <a href="#/devblog" class="nav-link" class:active={currentPage === 'devblog'} onclick={() => menuOpen = false}>{t('ui.nav_blog')}</a>
      <a href="#/stats" class="nav-link" class:active={currentPage === 'stats'} onclick={() => menuOpen = false}>{t('ui.nav_stats')}</a>
      ...
      {#if availableLocales.length > 1}
        <select class="locale-picker" onchange={(e) => setLocale((e.target as HTMLSelectElement).value)}>
